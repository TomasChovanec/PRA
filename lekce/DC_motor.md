...
1. Vytvořte funkce ```vpred(int rychlost)```, ```vzad(int rychlost)```, ```vlevo(int rychlost)```, ```vpravo(int rychlost)``` a ```stop()```. [Jak se tvoří vlastní funkce](Definice_funkce.md)
1. Napište program, kde robot pojede 2s rychle dopředu, pak se zastaví, otočí o 180° a pojede pomalu zpět.
