...
3. Zopakujte si, jak se tvoří [vlastní funkce](Definice_funkce.md)Vytvořte si funkce ```stepForward(int numSteps)``` - točí motorem po směru hodinových ručiček a ```stepBackward(int numSteps)``` - točí motorem proti směru hodinových ručiček. Argument ```int numSteps``` bude udávat, o kolik kroků se má motor otočit.
